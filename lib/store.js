...
async function storageDelete(path) {
  ...
  if (!res.ok && res.status !== 404) {
    const body = await res.text();
    throw new Error('Storage delete failed: ' + res.status + ' ' + body.slice(0, 200));
  }
}
...
async function deleteOrder(orderRef) {
  if (!orderRef) return;

  if (HAS_SUPABASE) {
    await storageDelete(orderPath(orderRef));
    return;
  }

  if (!HAS_KV) {
    memory.delete('order:' + orderRef);
    return;
  }

  await kv(['DEL', 'order:' + orderRef]);
}
